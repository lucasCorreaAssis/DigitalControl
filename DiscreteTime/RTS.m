...
ylabel('Amplitude');
legend('Referência');
title('Referencia - POLOS DOMINANTES');
grid on;
...
title('Sinal de controle - POLOS DOMINANTES');
grid on;
...
ylabel('Amplitude');
legend('Saida');
title('Resposta Controlador RTS - POLOS DOMINANTES')
grid on;
...
den = (poly2sym(Az, z)*poly2sym(flip(Az), z));
eq = 1+(1/ro)*(num+den);
